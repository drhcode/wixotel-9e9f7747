...
export const createInvoiceEmail = (params: {
  ...
  const statusColors = {
    ...
    paid: { bg: '#dcfce7', border: '#10b981', text: '#166534', icon: '✓' },
    overdue: { bg: '#fee2e2', border: '#ef4444', text: '#991b1b', icon: '⚠️' },
    cancelled: { bg: '#f1f5f9', border: '#64748b', text: '#475569', icon: '✕' }
  };

  const statusColor = statusColors[params.status as keyof typeof statusColors] || statusColors.pending;

  const content = `<div style="text-align:center;margin-bottom:30px;"><div style="display:inline-block;background:linear-gradient(135deg,#1e40af 0%,#3b82f6 100%);color:white;padding:12px 24px;border-radius:50px;font-size:18px;font-weight:600;box-shadow:0 4px 12px rgba(59,130,246,0.3);">📄 Invoice</div></div><div style="display:flex;justify-content:space-between;align-items:start;margin-bottom:30px;flex-wrap:wrap;"><div style="flex:1;min-width:200px;"><h2 style="color:#1e293b;margin:0 0 5px 0;font-size:28px;font-weight:700;">INVOICE</h2><p style="color:#64748b;margin:0;font-size:14px;">Invoice Number</p><p style="color:#3b82f6;margin:5px 0 0 0;font-size:20px;font-weight:700;font-family:monospace;">${params.invoiceNumber}</p></div><div style="text-align:right;flex:1;min-width:200px;margin-top:10px;"><div style="display:inline-block;background:${statusColor.bg};color:${statusColor.text};padding:8px 16px;border-radius:20px;border:2px solid ${statusColor.border};font-weight:600;font-size:14px;">${statusColor.icon} ${params.status.toUpperCase()}</div></div></div><div style="background:linear-gradient(135deg,#f8fafc 0%,#f1f5f9 100%);padding:25px;border-radius:12px;margin:30px 0;border:2px solid #e2e8f0;"><div style="display:grid;grid-template-columns:1fr 1fr;gap:25px;"><div><p style="color:#64748b;margin:0 0 5px 0;font-size:13px;font-weight:600;text-transform:uppercase;letter-spacing:0.5px;">Bill To</p><p style="color:#1e293b;margin:0;font-size:18px;font-weight:700;">${params.hotelName}</p>${params.hotel.address ? `<p style="color:#64748b;margin:5px 0 0 0;font-size:14px;line-height:1.5;">${params.hotel.address}${params.hotel.city ? `, ${params.hotel.city}` : ''}${params.hotel.country ? `, ${params.hotel.country}` : ''}</p>` : ''}</div><div style="text-align:right;"><p style="color:#64748b;margin:0 0 5px 0;font-size:13px;font-weight:600;text-transform:uppercase;letter-spacing:0.5px;">Invoice Details</p><table style="width:100%;margin-top:10px;"><tr><td style="text-align:right;padding:4px 0;color:#64748b;font-size:14px;">Issue Date:</td><td style="text-align:right;padding:4px 0;color:#1e293b;font-size:14px;font-weight:600;">${params.issueDate}</td></tr><tr><td style="text-align:right;padding:4px 0;color:#64748b;font-size:14px;">Due Date:</td><td style="text-align:right;padding:4px 0;color:#${params.status === 'overdue' ? 'ef4444' : '1e293b'};font-size:14px;font-weight:600;">${params.dueDate}</td></tr></table></div></div></div><div style="background:linear-gradient(135deg,#eff6ff 0%,#dbeafe 100%);border-left:4px solid #3b82f6;padding:25px;border-radius:8px;margin:30px 0;"><h3 style="color:#1e40af;margin:0 0 15px 0;font-size:18px;">Billing Period</h3><p style="color:#1e293b;margin:0;font-size:16px;font-weight:600;">${params.billingPeriodStart} <span style="color:#64748b;font-weight:400;">to</span> ${params.billingPeriodEnd}</p></div><div style="background:white;border:2px solid #e2e8f0;border-radius:12px;overflow:hidden;margin:30px 0;"><table style="width:100%;border-collapse:collapse;"><thead><tr style="background:linear-gradient(135deg,#1e293b 0%,#334155 100%);"><th style="padding:15px 20px;text-align:left;color:white;font-size:14px;font-weight:600;text-transform:uppercase;letter-spacing:0.5px;">Description</th><th style="padding:15px 20px;text-align:right;color:white;font-size:14px;font-weight:600;text-transform:uppercase;letter-spacing:0.5px;">Amount</th></tr></thead><tbody><tr style="border-bottom:1px solid #e2e8f0;"><td style="padding:20px;color:#475569;font-size:15px;">Subscription Service</td><td style="padding:20px;text-align:right;color:#1e293b;font-size:15px;font-weight:600;">${params.currency}${params.amount.toFixed(2)}</td></tr><tr style="border-bottom:1px solid #e2e8f0;"><td style="padding:20px;color:#475569;font-size:15px;">Tax</td><td style="padding:20px;text-align:right;color:#1e293b;font-size:15px;font-weight:600;">${params.currency}${params.taxAmount.toFixed(2)}</td></tr><tr style="background:linear-gradient(135deg,#f0f9ff 0%,#e0f2fe 100%);"><td style="padding:25px 20px;color:#1e40af;font-size:18px;font-weight:700;text-transform:uppercase;">Total Amount</td><td style="padding:25px 20px;text-align:right;color:#1e40af;font-size:24px;font-weight:700;">${params.currency}${params.totalAmount.toFixed(2)}</td></tr></tbody></table></div>${params.notes ? `<div style="background:#fef3c7;border-left:4px solid #f59e0b;padding:20px;border-radius:8px;margin:25px 0;"><p style="color:#92400e;margin:0 0 5px 0;font-size:14px;font-weight:600;">📌 Notes:</p><p style="color:#78350f;margin:0;font-size:14px;line-height:1.6;">${params.notes}</p></div>` : ''}`;

  return getBaseEmailTemplate(content, params.hotel, true);
};
